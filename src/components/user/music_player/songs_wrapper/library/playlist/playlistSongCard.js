...
export default (props)=>{
    ...
    const toDelete=()=>{
        setAnchorEl(null);
        db.collection("playlistSong").where("playlistName","==",props.playlistName)
        .where("name","==",props.data.name).get()
        .then((querySnapshot)=>{
    ...
    return(
        <Card className={classes.card} square onMouseOver={()=>{setOver(true)}} onMouseLeave={()=>{setOver(false)}}>
           <CardContent>
              ...
              {over && <PlayArrowIcon className={classes.musicIcon} /> }
               ...
               <div className={classes.linkName} onClick={cardDetails}>{props.data.name}</div>
               </Link>
               </div>
               </div>
              { over && <MoreHorizIcon className={classes.menuIcon} onClick={handleMenuOpen}/>}
              <Menu
                  anchorEl={anchorEl}
                  keepMounted
                  open={open}
